...
const StoryNotes = ({ postId, postTitle }) => {
    ...
    return (
        <div className="story-notes-widget" style={{
            ...
            background: 'var(--bg-secondary)', marginBottom: 24, overflow: 'hidden'
        }}>
            <button onClick={() => setOpen(!open)} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    {totalNotes > 0 && (
                        <span style={{ background: 'var(--primary-color)', color: 'white', borderRadius: 20, padding: '2px 8px', fontSize: 11, fontWeight: 700 }}>
                            {totalNotes}
                        </span>
                    )}
                </div>
                    ...
                    <div style={{ marginBottom: 16 }}>
                        <input
                            ...
                            style={{
                                ...
                                color: 'var(--text-color)', fontSize: 13, fontStyle: 'italic'
                            }}
                        />
                        <div style={{ position: 'relative' }}>
                            <textarea
                                ...
                                style={{
                                    ...
                                    border: '1px solid var(--border-color)', background: 'var(--bg-primary)',
                                    color: 'var(--text-color)', fontSize: 14, resize: 'vertical', boxSizing: 'border-box'
                                }}
                            />
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
                            ...
                            {COLORS.map(c => (
                                <button key={c} onClick={() => setSelColor(c)} style={{
                                    width: 18, height: 18, borderRadius: '50%', background: c, border: selColor === c ? '2px solid var(--text-color)' : '2px solid transparent', cursor: 'pointer', padding: 0
                                }} />
                            ))}
                    ...
                    {notes.length > 2 && (
                        <input
                            ...
                            style={{
                                ...
                                border: '1px solid var(--border-color)', background: 'var(--bg-primary)',
                                color: 'var(--text-color)', fontSize: 13, boxSizing: 'border-box'
                            }}
                        />
                    )}
                        ...
                        {filtered.map(note => (
                            <div key={note.id} style={{
                                ...
                                {editingId === note.id ? (
                                    <div>
                                        <textarea
                                            ...
                                            rows={3}
                                            style={{ width: '100%', padding: '6px', borderRadius: 6, border: '1px solid var(--border-color)', background: 'var(--bg-secondary)', color: 'var(--text-color)', fontSize: 13, resize: 'vertical', boxSizing: 'border-box' }}
                                        />
                                        <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
                                            <button onClick={() => saveEdit(note.id)} style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'var(--primary-color)', color: 'white', border: 'none', borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontSize: 12 }}>
                                                <Save size={12} /> Save
                                            </button>
                                            <button onClick={() => setEditingId(null)} style={{ background: 'none', border: '1px solid var(--border-color)', borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontSize: 12, color: 'var(--text-secondary)' }}>Cancel</button>
                                        </div>
                                    </div>
                                ) : (
                                    <p onClick={() => startEdit(note)} style={{ fontSize: 14, color: 'var(--text-color)', lineHeight: 1.5, margin: 0, cursor: 'text', whiteSpace: 'pre-wrap' }}>{note.text}</p>
                                )}
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 8 }}>
                                    ...
                                    <button onClick={() => deleteNote(note.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-secondary)', padding: 2 }}>
                                        <Trash2 size={13} />
                                    </button>
                                </div>
                            </div>
